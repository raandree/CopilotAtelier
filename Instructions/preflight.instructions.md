...
1. **Read the Memory Bank.** If `.memory-bank/` exists in the workspace, read the always-loaded set: `projectbrief.md`, `activeContext.md`, `techContext.md`, `progress.md`, `systemPatterns.md`, and `promptHistory.md` if present. If the folder does not exist, note it in the acknowledgment.
   - **Probe directly; do not trust the workspace summary.** The `<workspace_info>` / workspace structure listing surfaced at session start frequently omits dotfile folders (`.memory-bank`, `.git`, `.vscode`, etc.). Treating that summary as authoritative is a known failure mode that has caused agents to skip the Memory Bank entirely. Always verify with an explicit probe (`list_dir` on the workspace root, `file_search` for `.memory-bank/**`, or `Test-Path .memory-bank`) before concluding the Memory Bank is absent.
