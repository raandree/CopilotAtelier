...
3. **Match skills** in the `<skills>` block by description against the user's task, and read `SKILL.md` for any match (e.g. `german-legal-research`).
4. **Append a one-line entry** to `.memory-bank/promptHistory.md` if the file exists: `YYYY-MM-DD HH:mm UTC | legal-researcher | <one-line intent>`.
