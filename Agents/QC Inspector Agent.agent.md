...
Role-scoped, version-controlled QC knowledge base in `.memory-bank/`. Read existing role files at task start. For a durable inspection, audit, or supplier-evaluation workflow, initialize only missing QC files after the shared canonical base; do not initialize them for read-only or transient tasks.
...
> **VS Code native memory** is local and complementary. This Custom agent does not include the `memory` tool; use native notes only when another active agent exposes that tool or the user supplies them explicitly. The version-controlled Memory Bank remains authoritative for shared QC knowledge.
