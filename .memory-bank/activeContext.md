...
Repaired the `tools:` frontmatter of all 11 custom agents ([`Agents/`](../Agents/)): none could previously create a file or folder because every agent declared only `edit/editFiles` from the edit group. Added `edit/createFile` + `edit/createDirectory` to all eleven; `execute/runTask` to the four engineering/troubleshooting agents (Software Engineer, Security & QA, Troubleshooter, research-analyst); and `edit/rename` to the two refactor/fix-heavy agents (Software Engineer, Troubleshooter). The `browser/*` Playwright group was deliberately withheld to avoid the lethal trifecta (private data × untrusted web content × outbound channel). Verified: every `tools:` array parses as JSON (no duplicates, `edit/editFiles` retained), full YAML frontmatter valid, `markdownlint-cli2` 0 errors. Committed on `ai/fix-agent-tools` (not pushed). **Next step**: user re-runs `Setup-CopilotSettings.ps1` to deploy; tool changes take effect only when an agent is reselected/reloaded.

## Recent changes (2026-07-11 — agent tool-set repair)

- **All 11 agents gained file/folder creation.** Baseline `edit/createFile` + `edit/createDirectory` added to every `Agents/*.agent.md` (placed right after `edit/editFiles`; no other array entries reordered). The four engineering/troubleshooting agents — [Software Engineer](../Agents/Software%20Engineer%20Agent.agent.md), [Security & QA](../Agents/Security%20&%20Quality%20Assurance%20Agent.agent.md), [Troubleshooter](../Agents/Technical%20Troubleshooter%20Agent.agent.md), [research-analyst](../Agents/research-analyst.agent.md) — also gained `execute/runTask`; Software Engineer and Troubleshooter additionally gained `edit/rename`. Tool counts: SE 41→45, Troubleshooter 37→41, Security & QA 35→38, research-analyst 35→38, the six writer/research agents + QC Inspector +2 each. `browser/*` withheld (lethal-trifecta avoidance); notebook tools left untouched (only Software Engineer has them). Verified via JSON-array parse (well-formed, no dupes, `edit/editFiles` retained), full YAML frontmatter parse (name + tools present), and `markdownlint-cli2` (0 errors across 11 files). `edit/rename` could not be confirmed against a live tool picker; included per the user's audited canonical list for the two refactor-heavy agents (fails open if unknown).

### Prior focus — 2026-07-09 discipline-skills adoption (still uncommitted on its own branch)

Completed the full section-2 adoption from the 2026-07-09 external skills-repo review (logged in `promptHistory.md`): the behavioural-enforcement patterns in [`skill-creator`](../Skills/skill-creator/SKILL.md), the [`Reference/definition-of-done.md`](../Reference/definition-of-done.md) standing bar wired into the post-flight hook, and now the three domain-neutral discipline skills — [`test-driven-development`](../Skills/test-driven-development/SKILL.md), [`debugging-and-error-recovery`](../Skills/debugging-and-error-recovery/SKILL.md), [`code-review-and-quality`](../Skills/code-review-and-quality/SKILL.md) — each PowerShell/DSC-flavoured, dogfooding the enforcement pattern, fenced against the existing domain playbooks, and linking to the Definition of Done. Skill count 34 → 37. All changes are **not committed** (user: "Dont commit"). **Next step**: none outstanding — the optional follow-ups are done too: a `notes-evals.md` trigger set (four scenarios each) for all three skills, and the Software Engineer agent wired to point at them (Testing Strategy → `test-driven-development`, Error Recovery → `debugging-and-error-recovery`, Code Review → `code-review-and-quality`). Triggering was then validated twice: first a blind, description-only selection eval in two fresh subagent contexts (6/6 fences held, 11/12 intended triggers, 17/18 cross-run agreement), then — after the user deployed the skills via `Setup-CopilotSettings.ps1` — a stronger real-catalogue re-run with five fresh subagents (3/3 new skills fire, both fences hold, the security-audit prompt correctly returning `none` while citing the DO NOT USE FOR fences). A brand-new top-level-session test remains optional; the work is still **uncommitted**.

## Recent changes (2026-07-09 — three domain-neutral discipline skills)

Authored three new skills for the engineering disciplines an agent most often skips, filling the gaps identified in the review (original content; no external source credited):

- **[`test-driven-development`](../Skills/test-driven-development/SKILL.md)** (116 lines, desc 1021) — red-green-refactor, failing Pester test before code, test pyramid, DAMP-over-DRY, bug-fix-as-test-first, characterization tests. Fences: mocking → `pester-patterns`, failure diagnosis → `sampler-build-debug`, syntax → `pester.instructions.md`, execution → `powershell-execution-safety`.
- **[`debugging-and-error-recovery`](../Skills/debugging-and-error-recovery/SKILL.md)** (96 lines, desc 998) — reproduce → localize → reduce → fix-root-cause → guard; stop-the-line; do-not-swallow-errors; flaky-test discipline. Fences: `dsc-troubleshooting`, `winrm-troubleshooting`, `sampler-build-debug`, `long-running-job-monitor`, `agent-security-review`.
- **[`code-review-and-quality`](../Skills/code-review-and-quality/SKILL.md)** (98 lines, desc 976) — five axes (design/correctness/complexity/tests/clarity), severity labels, change sizing, author self-review. Fences: security → `code-review` prompt / `agent-security-review`, adversarial → `devils-advocate-review`, doc peer review → `peer-review` prompt.
- **Shared**: all three carry the anti-rationalization + red-flags + evidence-close pattern from `skill-creator` and link to the [Definition of Done](../Reference/definition-of-done.md). Created via placeholder-then-editor to keep embedded PowerShell fences out of shell parsing; normalised to LF + UTF-8 no-BOM.
- **Registered** in the README Available Skills table and `techContext.md` Skills inventory. Skill count 34 → 37.
- **Verification**: bodies 116/96/98 (≤ 500); descriptions 1021/998/976 (≤ 1024); `markdownlint-cli2` → 0 errors across all three.
- **Wired + evalled (same-day follow-up)** — added a `notes-evals.md` trigger set (four Claude-A/Claude-B scenarios) to each of the three skills, and pointed [`Agents/Software Engineer Agent.agent.md`](../Agents/Software%20Engineer%20Agent.agent.md) at them (Testing Strategy → `test-driven-development`, Implementation-Level Recovery → `debugging-and-error-recovery`, Subagent Code Review → `code-review-and-quality`); `markdownlint-cli2` → 0 errors on all four.
- **Triggering validated (two rounds)** — (1) a blind, description-only selection eval over the 12 eval prompts plus 6 decoys in two independent fresh subagent contexts: 6/6 decoy fences held, 11/12 intended triggers fired, 17/18 cross-run agreement (only the pre-flagged "the build fails" prompt flipped `debugging-and-error-recovery` ↔ `sampler-build-debug`). (2) After the user deployed via `Setup-CopilotSettings.ps1` (skills now in `~/.copilot/skills`; catalogue refreshed mid-session), a stronger real-catalogue re-run — five fresh subagents each selecting from the full ~40-skill catalogue: all three new skills fired on their intended prompts, and both fences held ("mock a REST API in my Pester test" → `pester-patterns`; "audit… vulnerabilities… CVSS" → `none`, the subagent explicitly citing the DO NOT USE FOR fences and routing to the security-reviewer agent / `code-review` prompt). No description changes needed.
