...
Authored a new skill **`long-running-job-monitor`** (skills 32 → **33**) governing how the agent runs, monitors, and reports on long-running live/integration tests, installers, and deployments, and added its execution-foundation anchor as `systemPatterns.md` **Decision 10** ("Long-running command execution"). Work is on `main`, **uncommitted** — the user said "dont commit". **Next step**: review the working-tree changes; commit + PR only on explicit request (nothing committed or pushed this turn).

## Recent changes (2026-07-07 — long-running-job-monitor skill, uncommitted)
...
- **Verification**: SKILL.md 173 lines (≤ 500), description 1006 chars (≤ 1024), folder name == `name:`, all three markdown files lint clean (`markdownlint-cli2`, 0 errors), the sidecar is PSScriptAnalyzer- and AST-clean. **Uncommitted** on `main` per "dont commit".
