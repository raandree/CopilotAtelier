...
| `windows-gui-screenshot-capture` | `Skills/windows-gui-screenshot-capture/` | Capture Windows GUI screenshots programmatically (WPF `RenderTargetBitmap`, WinForms `DrawToBitmap`, Win32 `PrintWindow`, WebView2 `CapturePreviewAsync`, Avalonia headless `CaptureRenderedFrame`, WinUI 3 `RenderTargetBitmap.RenderAsync` / `Windows.Graphics.Capture`) and assemble a screenshot-embedded Markdown manual; GPU-composited-returns-black rule, self-capturing scene mode, MessageBox `#32770` capture, STA/DPI/font gotchas. Ships `references/engine-recipes.md` + a verbatim `scripts/DialogCapture.ps1` |
