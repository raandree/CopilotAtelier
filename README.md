...
| **brand-logo-system** | Design a project's visual identity and render it as SVG masters plus deterministic PNG deliverables, then export the eleven-slot set a shared logo library expects. Covers recovering a palette and mark from a project's existing artwork by pixel count, composing primary lockups, glyph-only marks, app icons, splash screens and monochrome variants from one brand definition, headless-Edge rasterisation at exact canvas sizes, and a measured gate over count, naming, slot parity, dimensions, corner alpha, painted bounds, centring, and ink coverage at 32 and 16 px. Ships a definition-driven renderer. |
