...
| **windows-gui-screenshot-capture** | Capture screenshots of a Windows desktop GUI programmatically — using the capture API each rendering engine actually supports — and assemble them into a screenshot-embedded Markdown user manual, generalised from the `D:\guitest` proof-of-concept to real apps. Covers the capture-API-per-engine matrix (WPF `RenderTargetBitmap`, WinForms `Control.DrawToBitmap` sized to `Form.Size`, Win32/GDI `PrintWindow`, WebView2 `CapturePreviewAsync`, Avalonia headless `CaptureRenderedFrame`, WinUI 3 `RenderTargetBitmap.RenderAsync` / `Windows.Graphics.Capture`), the GPU-composited-returns-black rule, the self-capturing `-CaptureDir` / `--capture` scene mode that runs unattended, native MessageBox (`#32770`) capture, and the STA / DPI-unaware / `SizeToContent` / Avalonia-headless-font gotchas. Ships a per-engine recipe reference and a ready-to-use `DialogCapture.ps1`. |
