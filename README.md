...
| **agent-evals** | Measure Customization discovery, repeated-run reliability, and with/without quality, token, and duration differences. Starts with native VS Code tooling and Waza, with offline PowerShell grading as a fallback. The bundled gates validate case definitions, require complete numbered samples, match literal substrings, and reject malformed trigger replies instead of counting them as successful negatives. Prepare and Grade need no model backend; Prepare requires `powershell-yaml`. |
