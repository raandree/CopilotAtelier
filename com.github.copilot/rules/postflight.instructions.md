...
2. **Update the Memory Bank.** If `.memory-bank/` exists: overwrite `activeContext.md` when the focus changed; append a dated line to `progress.md` for a shipped change or recorded event, curating the oldest entries in the same edit when the file is at or near its line budget; record a durable pattern in `systemPatterns.md`; append the `promptHistory.md` line, creating the file if absent. This step owns the `promptHistory.md` append — pre-flight no longer writes it.
