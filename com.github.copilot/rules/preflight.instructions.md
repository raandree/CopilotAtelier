...
3. **Initialize only for durable work.** If a durable project/configuration write or explicit durable record is requested and `.memory-bank/` or a required version-controlled base file is missing, load `memory-bank`, create only missing files before the first project edit, and never overwrite existing content. The initializer may also create local `promptHistory.md`; an absent local log or optional `glossary.md` does not make a read-only checkout incomplete. Do not initialize for Q&A, clarification, read-only investigation, or transient personal preferences. After initialization, read the index and apply its routes. Create a role-specific Memory Bank file only when the active Custom agent declares it and the current durable task needs it. Never scaffold another agent's schema, and never pre-create a declared file this task does not use.
